...
byte mac[] = { 
  0xDE, 0xAD, 0xBE, 0xE4, 0xDE, 0x35 }; // MAC-адрес нашего устройства
byte ip[] = { 
  192, 168, 0, 21 };
byte subnet[] = { 
  255, 255, 255, 0 };
...
void loop() {
  ...
  while (addeeprom < 7) {
    valeeprom = EEPROM.read(addeeprom);
    digitalWrite(addeeprom,valeeprom);
    Serial.print(addeeprom);
    Serial.print(": ");
    Serial.println(valeeprom);
    pineeprom = pineeprom + 1;
    addeeprom = addeeprom + 1;
  }
}
